...
class TestExecutionHooksHint:
    ...
    def test_job_type(self, mocker):
        import os

        from dirac_cwl_proto.commands.job_types import TestingJobType

        with tempfile.TemporaryDirectory() as _dir:
            workflowMock = mocker.Mock()
            workflowMock.job_path = _dir

            hint = ExecutionHooksHint(configuration={"job_type": "TestingJobType"})
            plugin = hint.to_runtime()

            # It instantiated the correct JobType
            assert isinstance(plugin.job_type_processor, TestingJobType)

            outputs = [
                "Autogenerated Output 1",
                "Autogenerated Output 2",
                "foo",
                "bar",
            ]
            filenamesOutputs = []
    ...
    def test_job_type_not_found(self, mocker):
        hint = ExecutionHooksHint(configuration={"job_type": "NonExistentJobType"})
        with pytest.raises(ValueError):
            hint.to_runtime()
