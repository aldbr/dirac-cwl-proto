...
class RequirementValidator(BaseModel, ABC):
    ...
    def validate_requirements(self, cwl_object=None) -> None:
        """
        ...
        if not isinstance(cwl_object, CommandLineTool) and cwl_object.steps:
            self.validate_steps_requirements(cwl_object.steps, global_requirement)

    def validate_steps_requirements(self, steps, global_requirement):
        """
